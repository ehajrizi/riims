...
export default observer(function AnetaresiaForm() {
    ...
    function handleFormSubmitAnetaresia(anetaresia: Anetaresia) {
        if (anetaresia.id.length === 0) {
            ...
            createAnetaresia(newAnetaresia).then(() => history.push(`/anetaresia`))
    ...
    return (
        <Segment clearing>
            <Header content='Anetaresia' sub color='blue' />
        <Formik
            ...
            {({ handleSubmit, isValid, isSubmitting, dirty }) => (
                <Form className='ui form' onSubmit={handleSubmit} autoComplete='off'>
                    ...
                    <Button onClick={()=>modalStore.closeModal()} as={Link} to='/anetaresia' floated='right' type='button' content='Cancel' />
                </Form>
            )}
        </Formik>
    </Segment >
